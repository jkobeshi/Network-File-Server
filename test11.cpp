...
int main(int argc, char *argv[]) {
    ...
    status = fs_create("user1", "/dir/dir2/file2", 'f');
    assert(!status);

    status = fs_create("user1", "/dir/dir3", 'd');
    assert(!status);

    status = fs_create("user1", "/dir/dir3/file", 'f');
    assert(!status);

    status = fs_create("user1", "/dir/dir3/file2", 'f');
    assert(!status);

    status = fs_create("user1", "/dir/dir2/dir4", 'd');
    assert(!status);

    status = fs_create("user1", "/dir/dir2/dir4/file", 'f');
    assert(!status);

    status = fs_create("user1", "/dir/dir2/dir4/file2", 'f');
    assert(!status);

    status = fs_create("user1", "/dir/dir3/dir4", 'd');
    assert(!status);

    status = fs_create("user1", "/dir/dir3/dir4/file", 'f');
    assert(!status);

    status = fs_create("user1", "/dir/dir3/dir4/file2", 'f');
    
    status = fs_writeblock("user1", "/dir/dir2/dir4/file", 0, writedata);

    status = fs_readblock("user1", "/dir/dir2/dir4/file", 0, readdata);

    status = fs_writeblock("user1", "/dir/dir3/dir4/file2", 1, writedata);

    status = fs_writeblock("user1", "/dir/dir3/dir4/file2", 1, writedata);

    status = fs_writeblock("user1", "/dir/dir3/dir4/file2", 1, writedata);

    status = fs_readblock("user1", "/dir/dir3/dir4/file2", 1, readdata);

    status = fs_readblock("user1", "/dir/dir3/dir4/file2", 1, readdata);

    status = fs_readblock("user1", "/dir/dir3/dir4/file2", 1, readdata);

    status = fs_writeblock("user1", "/dir/dir2/dir4/file", 2, writedata);

    status = fs_readblock("user1", "/dir/dir2/dir4/file", 2, readdata);
    ...
    status = fs_writeblock("user1", "/dir/dir3/dir4/file2", 3, writedata);

    status = fs_writeblock("user1", "/dir/dir3/dir4/file2", 3, writedata);

    status = fs_writeblock("user1", "/dir/dir3/dir4/file2", 3, writedata);

    status = fs_readblock("user1", "/dir/dir3/dir4/file2", 3, readdata);
