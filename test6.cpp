...
int main(int argc, char *argv[]) {
    ...
    status = fs_create("u2", "/3dA/dB/f2", 'f');
    assert(status);
    status = fs_create("u3", "/3dA/dB/f1", 'f');
    assert(status);
    status = fs_create("u3", "/f1", 'f');
    //assert(status);
    status = fs_create("u3", "/3dA/dB/f1", 'f');
    assert(status);
    status = fs_delete("u1", "/3dA/dB/f1");
    assert(status);
    status = fs_writeblock("u1", "/3dA/dB/f1", 0, writedata);
    assert(status);
    status = fs_readblock("u1", "/3dA/dB/f1", 1, readdata);
    assert(status);
